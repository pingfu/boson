...
Open questions and judgement calls carry a **[Q]** tag, some inline, all consolidated in §22. Decisions taken below are defaults, not commitments — push back on any of them.
...
## 21. Registry mode: running images GitHub built

Boson runs a project two ways. **Source mode** fetches the repo and builds on the host (§6 step 6, `up -d --build`). **Registry mode** fetches the repo for its compose file, pulls an image CI already built, and starts it. The compose file decides which.
...
After the fetch (§6 step 4) the deployer runs `docker compose config --format json` in the checkout, with `BOSON_IMAGE_TAG` already set (below) so tag interpolation resolves. A compose file where no service declares `build` is registry mode; anything else is source mode. The result is written to `projects.deploy_mode` at the end of the pass, and the webhook receiver reads that stored value to decide which event to act on (§14).
...
Pinning to the sha makes the compose file and the image come from the same commit, and makes "deploy sha X" a complete instruction. That is what a `boson rollback` needs (§22 [Q7]).

### Registry authentication

The installation token mints the same way (§13) and authenticates to GHCR as well as to git:

```bash
docker login ghcr.io -u x-access-token --password-stdin   # token on stdin
```

`ProcessRunner.RunAsync` already takes `stdin`, so the token never reaches argv here (§13's caveat applies to the git fetch URL only).

`docker login` writes credentials to `$DOCKER_CONFIG/config.json`, which §13's contract forbids. The narrower contract registry mode upholds: the deployer creates `/var/lib/boson/docker/<deploy-id>` with mode 0700, points `DOCKER_CONFIG` at it for the login and every compose invocation in the pass, and deletes the directory when the pass ends, on success and on failure. The token reaches disk only inside that directory, owned by the `boson` user, for the length of one deploy.

The App manifest declares `packages: read` (§11). GHCR grants an installation token access to a package only when the package inherits access from the repo the App is installed on, which is the default for images a repo's own workflow pushes. A package created outside that path, or with inherited access removed, fails the login with nothing to indicate the cause is a package setting rather than the token.
...
### Manifest, schema and pipeline changes

**§11 manifest.** Both events and the extra permission ship from the start, so no App is edited when a repo changes mode:
...
**§4 schema**, `Resources/migrations/0002_registry_mode.sql`:
...
**§6 step 6** in registry mode, with `BOSON_IMAGE_TAG` and `DOCKER_CONFIG` in the environment:
...
| `docker login ghcr.io` fails (`packages: read` missing, package access not inherited from the repo) | Deployer, before step 6 | Deploy row `failed`, `error='registry login: <detail>'`. Cause is App-level and won't self-heal. |
| Image tag absent (publish still in flight, or the workflow published a different tag) | Deployer step 6 | Compose pull fails, deploy row `failed`, log names the missing reference. Old container keeps running. |
...
**Every deploy leaves the previous sha's image on the host.** Nothing prunes them, so disk fills at roughly one image per deploy until an operator runs `docker image prune`.
...
Source mode is the lower floor: a repo with a Dockerfile and a compose file deploys with no pipeline of its own.
...
- **[Q7]** `boson rollback` — not in v1. The primitives exist (`reset --hard` to a SHA, `deploys.commit_sha` history); the objection is that it lets the host and the repo disagree with nothing recorded on GitHub's side. Registry mode (§21) answers that objection for the projects using it: the image published for a sha is a GitHub-side record of what runs, and `BOSON_IMAGE_TAG` makes a sha a complete deploy instruction. Source mode still has only the host's word for it. Revisit once registry mode has operational history.
- **[Q8]** Registry mode's publishing workflow is fixed at `.github/workflows/deploy.yml` (§21). A `projects.deploy_workflow` column plus `boson add --deploy-workflow` would make it configurable; defer until a repo needs a different filename.
- **[Q9]** Registry-mode image retention (§21) — boson prunes nothing, so old sha-tagged images accumulate. Pruning images no compose project references is a candidate for the end of a deploy pass; defer until a host actually fills.
